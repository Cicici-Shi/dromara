<template><div><h1 id="回顾" tabindex="-1"><a class="header-anchor" href="#回顾" aria-hidden="true">#</a> 回顾</h1>
<p>在 Soul 请求处理概览概览这篇文章中，我们已经知晓了 Soul 针对于请求的处理入库在<strong>DefaultSoulPluginChain 的 excute</strong>，其中执行了一个插件链的模式来完成了请求的处理。</p>
<p>我们大体梳理了注入到<strong>plugins</strong>的插件，但是即使这样依然不能纵观全局，对此特地对 soul 插件所涉及的类进行了相关梳理，整体梳理结果如下图。</p>
<figure><img src="https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/6d9c8e69429e4cb1bcc5bd54ad4f6112~tplv-k3u1fbpfcp-watermark.image" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>
<p>在梳理文章中可以看到核心类是<strong>SoulPlugin、PluginEnum、PluginDataHandler、MetaDataSubscriber</strong>，在梳理请求的相关文章中我们目前只需要重点关注 SoulPlugin 与 PluginEnum 类。</p>
<p>SoulPlugin 类我们已经有了一定的理解，那 PluginEnum 枚举类的主要作用是什么呢？</p>
<p>PluginEnum:插件的枚举类</p>
<table>
<thead>
<tr>
<th>属性</th>
...
<tr>
<td>code</td>
<td>插件的执行顺序 越小越先执行</td>
</tr>
<tr>
<td>role</td>
<td>角色 暂时未发现实际引用地址</td>
</tr>
<tr>
<td>name</td>
<td>插件名称</td>
...
<p>其实我们不难发现在<strong>DefaultSoulPluginChain 的 plugins</strong>的插件都是有固定的执行顺序的，那这个插件的执行顺序是在哪定义的呢？</p>
<p>最终可以追溯到<strong>SoulConfiguration</strong>类下</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>整理整个 PluginEnum 类相关引用，整理出如下表格，不难看出插件与插件之间的顺序关系<br>
<img src="https://p9-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/d89a3b45058846be94aa0b8935ec1868~tplv-k3u1fbpfcp-watermark.image" alt="" loading="lazy"></p>
<table>
<thead>
<tr>
<th>等级</th>
...
<tr>
<td>第一等级</td>
<td>只有 GlobalPlugin 全局插件</td>
</tr>
<tr>
<td>第二等级到第八等级</td>
<td>可以理解为在请求发起前的前置处理插件</td>
</tr>
<tr>
<td>第九等级到第十一等级</td>
<td>可以理解为针对调用方的方式所针对的不同调用处理</td>
</tr>
<tr>
<td>第十二等级</td>
<td>只有 MonitorPlugin 监控插件</td>
</tr>
<tr>
<td>第十三等级</td>
<td>是针对于各个调用方返回结果处理的 Response 相关插件</td>
</tr>
</tbody>
</table>
<p>在刚才的回顾中我们已经明白 soul 处理请求的大体流程</p>
<ul>
<li>1.GloBalPlugin 插件 进行全局的初始化</li>
<li>2.部分插件根据鉴权、限流、熔断等规则对请求进行处理</li>
<li>3.选择适合自己的调用方式进行拼装参数，发起调用。</li>
<li>4.进行监控</li>
<li>5.对调用的结果进行处理</li>
</ul>
<h1 id="请求流程梳理" tabindex="-1"><a class="header-anchor" href="#请求流程梳理" aria-hidden="true">#</a> 请求流程梳理</h1>
<blockquote>
<p>以下演示代码截图来自于 soul-examples 下的 http demo，调用的接口地址为<a href="http://127.0.0.1:9195/http/test/findByUserId?userId=10" target="_blank" rel="noopener noreferrer">http://127.0.0.1:9195/http/test/findByUserId?userId=10<ExternalLinkIcon/></a></p>
</blockquote>
<p>在<strong>DefaultSoulPluginChain 的 excute</strong>方法进行埋点，查看一次 http 请求调用经过了哪些类？</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token class-name">Mono</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">Void</span><span class="token punctuation">></span></span> <span class="token function">execute</span><span class="token punctuation">(</span><span class="token keyword">final</span> <span class="token class-name">ServerWebExchange</span> exchange<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        ...
        <span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>最终输出的未跳过的插件如下：</p>
<p>未跳过的插件为 global.GlobalPlugin<br><br>
未跳过的插件为 sign.SignPlugin<br><br>
未跳过的插件为 waf.WafPlugin<br><br>
未跳过的插件为 ratelimiter.RateLimiterPlugin<br><br>
未跳过的插件为 hystrix.HystrixPlugin<br><br>
未跳过的插件为 resilience4j.Resilience4JPlugin<br><br>
未跳过的插件为 divide.DividePlugin<br><br>
未跳过的插件为 httpclient.WebClientPlugin<br><br>
未跳过的插件为 alibaba.dubbo.param.BodyParamPlugin<br><br>
未跳过的插件为 monitor.MonitorPlugin<br><br>
未跳过的插件为 httpclient.response.WebClientResponsePlugin<br></p>
<blockquote>
<p>这里有个小疑惑，为啥这个 alibaba.dubbo.param.BodyParamPlugin 插件会被执行，暂时忽略，后期跟踪。</p>
</blockquote>
<p>我们发现一次针对于 http 请求的网关调用 所执行的插件的大体流程与我们猜想的处理流程一致。<br><br>
目前我们只挑重点来讲，即<strong>GlobalPlugin、DividePlugin、WebClientPlugin、WebClientResponsePlugin</strong>。</p>
<p>发起 Debug 调用依次追踪上述四个插件的作用。</p>
<h2 id="globalplugin-soulcontext-对象封装插件" tabindex="-1"><a class="header-anchor" href="#globalplugin-soulcontext-对象封装插件" aria-hidden="true">#</a> GlobalPlugin SoulContext 对象封装插件</h2>
<p>GlobalPlugin 的插件的 excute 方法如下所示</p>
...
<tr>
<td>module</td>
<td>每种 RPCType 针对的值不同 http 调用时指代网关调用的前置地址</td>
</tr>
<tr>
<td>method</td>
<td>切割后的方法名（在 RpcType 为 http 时）</td>
</tr>
<tr>
<td>rpcType</td>
<td>RPC 调用类型有 Http、dubbo、sofa 等</td>
</tr>
<tr>
<td>httpMethod</td>
<td>Http 调用的方式目前只支持 get、post</td>
</tr>
<tr>
<td>sign</td>
<td>鉴权的相关属性目前不知道具体作用，可能与 SignPlugin 插件有关</td>
</tr>
<tr>
<td>timestamp</td>
<td>时间戳</td>
...
<tr>
<td>path</td>
<td>路径指代调用到 soul 网关的全路径（在 RpcType 为 http 时）</td>
</tr>
<tr>
<td>contextPath</td>
<td>与 module 取值一致（在 RPCType 为 http 时）</td>
</tr>
<tr>
<td>realUrl</td>
<td>与 method 的值一致（在 RpcType 为 http 时）</td>
</tr>
<tr>
<td>dubboParams</td>
<td>dubbo 的参数？</td>
...
<tr>
<td>startDateTime</td>
<td>开始时间怀疑与监控插件和统计指标模块有联用</td>
</tr>
</tbody>
</table>
<p>在执行完 GlobalPlugin 插件后，最终封装完成的 SoulContext 对象如下所示。<br>
<img src="https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/d34d9e900a1e4448b8f15302db36a5bb~tplv-k3u1fbpfcp-watermark.image" alt="" loading="lazy"></p>
<p>其他 RPCType 的 SoulContext 的参数封装可以查看<strong>DefaultSoulContextBuilder 的 build</strong>方法进行追踪，由于本编文章主要追溯 http 调用，故在这里不在多余讨论。</p>
<h2 id="divideplugin-路由选择插件" tabindex="-1"><a class="header-anchor" href="#divideplugin-路由选择插件" aria-hidden="true">#</a> DividePlugin 路由选择插件</h2>
<p>在执行完成 GlobalPlugin 插件后，最终封装成了一个<strong>SoulContext 对象</strong>，并将其放在了<strong>ServerWebExchange</strong>中，供下游的调用链使用。</p>
<p>接下来让我们看一下<strong>DividePlugin 插件</strong>在整个链式调用过程中到底起了一个什么样的作用？</p>
<h3 id="abstractsoulplugin" tabindex="-1"><a class="header-anchor" href="#abstractsoulplugin" aria-hidden="true">#</a> AbstractSoulPlugin</h3>
<p>通过追溯源码得知<strong>DividePlugin 插件继承于 AbstractSoulPlugin 类，而 AbstractSoulPlugin 类实现了 SoulPlugin 接口</strong>。</p>
<p>那么<strong>AbstractSoulPlugin</strong>又做了哪些扩展呢？让我们梳理一下该类的方法。</p>
<table>
<thead>
<tr>
<th>方法名</th>
...
<tr>
<td>excute</td>
<td>实现于 SoulPlugin 接口，在 AbstractSoulPlugin 中起到一个<strong>模板方法的作用</strong></td>
</tr>
<tr>
<td>doexcute</td>
<td><strong>抽象方法</strong> 交由各个子类实现</td>
</tr>
<tr>
<td>matchSelector</td>
<td>匹配选择器</td>
...
<tr>
<td>handleSelectorIsNull</td>
<td>处理选择器为空情况</td>
</tr>
<tr>
<td>handleRuleIsNull</td>
<td>处理规则为空情况</td>
</tr>
<tr>
<td>selectorLog</td>
...
<p>看一下<strong>excute</strong>方法的具体作用</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token class-name">Mono</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">Void</span><span class="token punctuation">></span></span> <span class="token function">execute</span><span class="token punctuation">(</span><span class="token keyword">final</span> <span class="token class-name">ServerWebExchange</span> exchange<span class="token punctuation">,</span> <span class="token keyword">final</span> <span class="token class-name">SoulPluginChain</span> chain<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/a1ec8bd02d6546c79a96d67535049aae~tplv-k3u1fbpfcp-watermark.image" alt="" loading="lazy"></p>
<p>ps：在上述的流程图中并没有细化到具体的方法级别的处理。</p>
<p>但仍有几个点需要着重解释一下：</p>
<ul>
<li>1.插件数据、选择器数据、规则数据的获取全部来自于<strong>BaseDataCache</strong>，该类是数据同步过程中最终会影响的类。</li>
<li>2.选择器的类型，在使用 SpringMvc 项目进行接口注册时，会有一个 isFull 的选项为 true 代表全局代理，在全局代理模式下只会注册一个选择器\规则(指代代理所有的接口),所以这里的对应处理为 rule.size()-1.</li>
<li>3.选择器和规则的选择，实际的处理要复杂的多，考虑到是介绍一次请求流程的大体逻辑，所以这里不展开阐述，有兴趣的可以查看<strong>MatchStrategy、AbstractMatchStrategy 及其相关实现类</strong>（后期会单独开一篇具体讲解），此处对应页面的如下：<br>
<img src="https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f624b13f205a44e29b2799718433e0c9~tplv-k3u1fbpfcp-watermark.image" alt="" loading="lazy"><br>
<img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f590c2cf336442f08a52b864c81d41a8~tplv-k3u1fbpfcp-watermark.image" alt="" loading="lazy"></li>
</ul>
<p>梳理一下<strong>AbstractSoulPlugin 的 exeute 方法</strong>作用，经过上述流程图的引导，我们已经知晓该方法的作用是为了选取插件---&gt;选取选择器---&gt;选取规则，最后交由子类的<strong>doexcute</strong>方法。</p>
<p>接下来让我们看一下<strong>DividePlugin 的 doexcute</strong>方法具体做了哪些事。</p>
<h3 id="divideplugin" tabindex="-1"><a class="header-anchor" href="#divideplugin" aria-hidden="true">#</a> DividePlugin</h3>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">protected</span> <span class="token class-name">Mono</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">Void</span><span class="token punctuation">></span></span> <span class="token function">doExecute</span><span class="token punctuation">(</span><span class="token keyword">final</span> <span class="token class-name">ServerWebExchange</span> exchange<span class="token punctuation">,</span> <span class="token keyword">final</span> <span class="token class-name">SoulPluginChain</span> chain<span class="token punctuation">,</span> <span class="token keyword">final</span> <span class="token class-name">SelectorData</span> selector<span class="token punctuation">,</span> <span class="token keyword">final</span> <span class="token class-name">RuleData</span> rule<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        ...
        <span class="token keyword">return</span> chain<span class="token punctuation">.</span><span class="token function">execute</span><span class="token punctuation">(</span>exchange<span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>通过上述代码梳理完成后大体逻辑如下：</p>
<ul>
<li>1.获取选择器对应的注册地址，对应页面数据如下<br>
<img src="https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/bea039b5c98040ee80433f785dac85aa~tplv-k3u1fbpfcp-watermark.image" alt="" loading="lazy"></li>
<li>2.根据规则的 handle 字段获取负载均衡策略，并选择真实的调用地址（<strong>LoadBalanceUtils</strong>），重试次数和超时时间，对应页面数据如下。<br>
<img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/07db1f8f76fc43b2aec61ee0f9ca4c05~tplv-k3u1fbpfcp-watermark.image" alt="" loading="lazy"></li>
<li>3.将真实调用地址，超时时间，重试次数传递到<strong>ServerWebExchange</strong>中，供下游调用链使用。<br>
debug 演示：<br>
<img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/0aa5f7d95f2942169b33029f074f1712~tplv-k3u1fbpfcp-watermark.image" alt="" loading="lazy"><br>
ps：在上述的主题逻辑中我们没有看到参数在哪里？那这个参数在哪封装的呢？答案在<strong>buildRealURL 方法中</strong>，是从<strong>exchange</strong>上下文中获取到的。</li>
</ul>
<h2 id="webclientplugin-http-请求调用插件" tabindex="-1"><a class="header-anchor" href="#webclientplugin-http-请求调用插件" aria-hidden="true">#</a> WebClientPlugin Http 请求调用插件</h2>
<p>接下来让我们看看 Soul 如何发起的请求调用</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token class-name">Mono</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">Void</span><span class="token punctuation">></span></span> <span class="token function">execute</span><span class="token punctuation">(</span><span class="token keyword">final</span> <span class="token class-name">ServerWebExchange</span> exchange<span class="token punctuation">,</span> <span class="token keyword">final</span> <span class="token class-name">SoulPluginChain</span> chain<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        <span class="token keyword">final</span> <span class="token class-name">SoulContext</span> soulContext <span class="token operator">=</span> exchange<span class="token punctuation">.</span><span class="token function">getAttribute</span><span class="token punctuation">(</span><span class="token class-name">Constants</span><span class="token punctuation">.</span><span class="token constant">CONTEXT</span><span class="token punctuation">)</span><span class="token punctuation">;</span>
        <span class="token keyword">assert</span> soulContext <span class="token operator">!=</span> <span class="token keyword">null</span><span class="token punctuation">;</span>
    ...
    <span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在 webClient 的<strong>excute</strong>方法中，主要做了三个事</p>
<ul>
<li>1.将从 Divide 插件中放入 exchange 的属性取出来，<strong>调用的真实地址、超时时间、重试次数</strong>。</li>
<li>2.封装了一个<strong>RequestBodySpec</strong>对象（不认识这个响应式编程的东西）</li>
<li>3.调用了一个<strong>handleRequestBody</strong>方法</li>
</ul>
<p>先认识<strong>handleRequestBody</strong>方法</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">private</span> <span class="token class-name">Mono</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">Void</span><span class="token punctuation">></span></span> <span class="token function">handleRequestBody</span><span class="token punctuation">(</span><span class="token keyword">final</span> <span class="token class-name">WebClient<span class="token punctuation">.</span>RequestBodySpec</span> requestBodySpec<span class="token punctuation">,</span>
    ...
    <span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在这个方法里，大体可以理解为</p>
<ul>
<li>exchange 中的请求头放到本次调用的请求头中</li>
<li>设置 contentType</li>
<li>设置超时时间</li>
<li>设置失败响应</li>
<li>设置重试的场景及重试次数</li>
<li>最终结果的处理。<br>
在流程中需要还需要看一个<strong>doNext 方法</strong></li>
</ul>
<p>大体逻辑就是判断请求是否成功，将请求结果放入 exchange 中交给下游插件处理。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">private</span> <span class="token class-name">Mono</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">Void</span><span class="token punctuation">></span></span> <span class="token function">doNext</span><span class="token punctuation">(</span><span class="token keyword">final</span> <span class="token class-name">ClientResponse</span> res<span class="token punctuation">,</span> <span class="token keyword">final</span> <span class="token class-name">ServerWebExchange</span> exchange<span class="token punctuation">,</span> <span class="token keyword">final</span> <span class="token class-name">SoulPluginChain</span> chain<span class="token punctuation">)</span> <span class="token punctuation">{</span>
        ...
        <span class="token keyword">return</span> chain<span class="token punctuation">.</span><span class="token function">execute</span><span class="token punctuation">(</span>exchange<span class="token punctuation">)</span><span class="token punctuation">;</span>
    <span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>ps： 虽然并不懂响应式编程，但并不影响我们阅读代码。</p>
<h2 id="webclientresponseplugin-http-结果处理插件" tabindex="-1"><a class="header-anchor" href="#webclientresponseplugin-http-结果处理插件" aria-hidden="true">#</a> WebClientResponsePlugin Http 结果处理插件</h2>
<p>该实现的 excute 方法没有什么核心逻辑，就是判断请求状态码，根据状态码返回给前端不同的数据格式。</p>
<div class="language-java line-numbers-mode" data-ext="java"><pre v-pre class="language-java"><code><span class="token keyword">public</span> <span class="token class-name">Mono</span><span class="token generics"><span class="token punctuation">&lt;</span><span class="token class-name">Void</span><span class="token punctuation">></span></span> <span class="token function">execute</span><span class="token punctuation">(</span><span class="token keyword">final</span> <span class="token class-name">ServerWebExchange</span> exchange<span class="token punctuation">,</span> <span class="token keyword">final</span> <span class="token class-name">SoulPluginChain</span> chain<span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<p>到此为止，一个基于 Soul 网关发起的 Http 请求调用流程大体已经结束。</p>
<p>梳理 http 请求调用流程</p>
<ul>
<li>Global 插件封装 SoulContext 对象</li>
<li>前置插件处理熔断限流鉴权等操作。</li>
<li>Divide 插件选择对应调用的真实地址，重试次数，超时时间。</li>
<li>WebClient 插件发起真实的 Http 调用</li>
<li>WebClientResponse 插件处理对应结果，返回前台。</li>
</ul>
<p>基于 Http 调用的大体流程，我们可以大体猜测出基于别 RPC 调用的流程，就是替换发起请求的插件和返回结果处理的插件。</p>
<p>在上文中我们还提到了路由规则的选择<strong>LoadBalanceUtils</strong>，选择器和规则的处理<strong>MatchStrategy</strong>。</p>
<p>之后将会开启新篇章一步步揭开 RPC 泛化调用，路由选择，选择器、规则匹配的神秘面纱。</p>
</div></template>
